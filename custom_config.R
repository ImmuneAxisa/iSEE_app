...
load_object <- function(x) {
  ...
  if (length(x) == 2 && sum(grepl("\\.rds$", basename(x))) == 1 && sum(grepl("\\.h5$", basename(x))) == 1) {
    ...
    showModal(modalDialog(
      ...
      "Unless importing HDF5 saved data, select only one file",
      easyClose = TRUE
    ))
    #session$reload()
  ...
  createLandingPage(
    seUI = function(id)
      fileInput(id,
                HTML(
                  paste(
                    "path to Seurat or sce file (rds, qs, HDF5).",
                    "",
                    ...
                    "",
                    "If imported object is a Seurat object it will be converted to a sce object before starting the app",
                    sep = "<br/>"
                  )
                ),
                width = '200%',
    ...
    initUI = function(id)
      fileInput(id, "Initial state RDS file: (optional)",
                multiple = FALSE),
    initLoad = function(x) readRDS(x$datapath)[["memory"]]
  )
